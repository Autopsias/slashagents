...
- NEVER create **init**.py files that break existing imports
...
- **Package Structure**: **init**.py files, namespace packages
...
2. Fix package structure with proper **init**.py files
...
2. **Check Package Structure**: Verify **init**.py files and package hierarchy
...
# Check for missing **init**.py files

find src -name "_.py" -path "_/services/*" -exec dirname {} \; | sort -u | xargs -I {} ls -la {}/**init**.py
...
class UserService:
    def **init**(self, auth_service: Optional['AuthService'] = None):
...
# Create all necessary **init**.py files for a Python project
...
touch src/models/**init**.py
touch src/utils/**init**.py
touch src/database/**init**.py
touch src/api/**init**.py

# Test package files

touch tests/**init**.py
touch tests/unit/**init**.py
touch tests/integration/**init**.py
touch tests/fixtures/**init**.py
...
- **models import structure**
  - Issue: Missing **init**.py causing import failures
  - Fix: Added **init**.py files to all package directories
  - Structure: src/models/**init**.py created
...
- **Proper Package Structure**: Always include **init**.py files
