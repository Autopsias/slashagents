...
tools: Read, Edit, MultiEdit, Bash, Grep, mcp**semgrep-hosted**security_check, SlashCommand
model: sonnet
color: red
---
...
def remediate_hardcoded_secrets():
    ...
    secret_patterns = [
        (r'API_KEY\s_=\s_["\']([^"\']+)["\']', 'API_KEY = os.getenv("API_KEY")'),
        (r'SECRET_KEY\s_=\s_["\']([^"\']+)["\']', 'SECRET_KEY = os.getenv("SECRET_KEY")'),
        (r'PASSWORD\s_=\s_["\']([^"\']+)["\']', 'PASSWORD = os.getenv("DATABASE_PASSWORD")')
    ]
...
def remediate_sql_injection():
    ...
    dangerous_patterns = [
        ...
        (r'f"SELECT._{._}"', 'parameterized_query_needed'),
        (r'query\s_=._\+.*', 'parameterized_query_needed'),
        (r'\.format\([^)]_\)._SELECT', 'parameterized_query_needed')
    ]
